      ...
      ws.value.onmessage = (event) => {
        try {
          ...
          if (data.type === 'chat') {
            ...
            const exists = messages.value[friendId].some((m: any) => m.id === data.id);
            if (!exists) {
            ...
            for (const friendId in messages.value) {
              const pending = messages.value[friendId as any as number].findIndex(
                (m: any) => m.temp_id === data.temp_id && m.isMe && !m.id
              );
              if (pending !== -1) {
                messages.value[friendId as any as number][pending].id = data.id;
                messages.value[friendId as any as number][pending].created_at = data.created_at;
                break;
              }
            }
