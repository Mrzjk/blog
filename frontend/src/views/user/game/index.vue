        ...
        <el-card class="glass-card mb-4" shadow="never">
          <template #header>
            <div class="card-header">
              <el-icon class="text-accent"><User /></el-icon>
              <h3 class="section-title">寻找玩伴</h3>
            </div>
          </template>
          <div class="search-user-section">
            ...
            <div class="search-results" v-if="searchedUsers.length > 0">
              <div class="result-item" v-for="user in searchedUsers" :key="user.id">
                ...
                <div class="actions">
                  <el-button type="primary" size="small" link @click="handleAddFriend(user.id)">添加好友</el-button>
                  <el-button type="success" size="small" link @click="$router.push(`/user/chat?id=${user.id}`)">聊天</el-button>
                </div>
              </div>
            </div>
...
import { Trophy, User, Search } from '@element-plus/icons-vue'
import GameCard from '@/components/user/GameCard.vue'
...
const handleSearchUsers = async () => {
  ...
  try {
    // Correctly fetching from search endpoint, assuming /auth/users handles it now or /auth/ with q
